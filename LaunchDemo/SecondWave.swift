...
import UIKit
import Foundation
class SecondWave: UIView {

    
    var wavesDisplayLink : CADisplayLink!
    var secondWavesLayer : CAShapeLayer!
    var secondWavesColor : UIColor!
    ...
    func setUpWave(){
        ...
        //波浪的颜色
        secondWavesColor = UIColor.init(red: 86/255.0, green: 202/255.0, blue: 139/255.0, alpha: 1.0)
        //波浪速度
        waveSpeed = CGFloat(1.0 / M_PI)
        if secondWavesLayer == nil {
            secondWavesLayer = CAShapeLayer()
            secondWavesLayer.fillColor = secondWavesColor.cgColor
        ...
        currentK = self.frame.size.height / 2.0
        //定时器=====也可以用Timer实现
